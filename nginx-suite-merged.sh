...
install_certificate(){ local hostname="$1"; log_info "安装证书..."; mkdir -p /etc/nginx/ssl/${hostname}; chmod 700 /etc/nginx/ssl; "$ACME_SH_BIN" --force --install-cert -d "$hostname" --key-file /etc/nginx/ssl/${hostname}/key.pem --fullchain-file /etc/nginx/ssl/${hostname}/fullchain.pem --ca-file /etc/nginx/ssl/${hostname}/chain.pem --reloadcmd "systemctl reload nginx"; log_success "证书安装完成"; }
...
prompt_backend_url(){
  ...
  local input_url="" host="" port=""
  read -p "请输入${service_name}完整URL(如 http://127.0.0.1:${example_port}，直接回车则改用IP+端口): " input_url
  if [ -z "$input_url" ]; then
    ...
    input_url="http://$host:$port"
  fi
  printf -v "$result_var" '%s' "$input_url"
}
...
provision_site(){
  ...
  if [ -z "$backend_url" ]; then log_error "后端 URL 不能为空"; exit 1; fi
  check_nginx
